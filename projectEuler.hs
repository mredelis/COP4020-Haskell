...
-- From class
primeList n = last (go n 1 2)
  where
    go n c i
      | c > n = []
      | otherwise = if isPrime i then i: go n (c+1) (i+1) else go n c (i+1)
...
adjacentNDigitsGreatestProduct n = maximum [product (take n (drop x (map charToDigit thousandDigitNum))) | x <- [0 .. (length thousandDigitNum - n)]]

-- from 1 to n: from 1 to 13
charToDigit c = fromEnum c - 48
...
-- 13. Large sum : one-hundred 50-digit numbers.


-- 14. Collatz Sequences
...
powerDigitSum' n = sum [ head (drop x (map charToDigit (show(2^n)))) | x <- [0 .. length (show (2 ^ n)) - 1]]
...
-- d number of digits
-- i index (starts at 3 because first two indexes are 1 & 1)
nDigitFibNumberIndex d = go d 3 1 1
  where
    go d i f s
...
-- Find if number n is the sum of consecutive squares: t->current running total; s->incrementing number to square
